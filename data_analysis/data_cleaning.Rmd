...
hist(token_row_sum[Louvain_member==13])
###
...
log_eff = rep(0,length(unique(prop_token_filter_add$address)))
...
# 计算na的占比
sum(is.na(log_eff))/length(log_eff)
...
Louvain_member[is.nan(ad_log_eff)]
log_eff[Louvain_member==6]
ad_log_eff[Louvain_member==6]
hist(ad_log_eff[!is.nan(ad_log_eff)])
```


```{r}
table(Louvain_member)
var(ad_log_eff[!is.nan(ad_log_eff)])

```
## anti-realistic
```{r}

log_eff_rev = rep(0,length(log_eff))
...
for (i in 1:max(Louvain_member)) {
  if (sum(Louvain_member==i)==1) {
    ...
    a = log_eff_rev[Louvain_member==i]
    a = a[!is.na(a)]
    ad_log_eff_rev[Louvain_member==i] = rep(mean(a,na.rm = TRUE),sum(Louvain_member==i))
  }
...
hist(ad_log_eff_rev[!is.nan(ad_log_eff)])
```
