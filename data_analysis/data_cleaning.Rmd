...
for (i in 1:dim(M_cit_full)[1]-1) {
  for (j in (i+1):dim(M_cit_full)[1]) {
    M_cit_full[j,i] = M_cit[i,j]
  }
}
...
dim(M_cit_full)
#########
M_token_full = M_token

for (i in 1:dim(M_token_full)[1]-1) {
  for (j in (i+1):dim(M_token_full)[1]) {
    M_token_full[j,i] = M_token[i,j]
  }
}
...
print(leiden_member)
table(community_leiden$membership)
...
K = prop_token_filter %>% filter(address == unique(address)[community$membership==2])
sum(K$voting_power)

# 150862255 128928770  91459.33 207912.1 110840.4
```
